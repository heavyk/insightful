...
function Ambition$now (nextSituation) {
  ...
  if (self.situation && self.situation[0] !== '/' && self.situation !== self.startingSituation) {
    self._debug('WARNING ' + self.namespace + ' is trying to now while already in a now situation: ' + self.situation)
  ...
  if (!self.exitStageLeft && nextSituation !== self.situation) {
    ...
    if (self.situations[nextSituation]) {
      ...
      if (self.situations[nextSituation]['>']) {
        // FIXME: if no '>' is provided, we never switch situations
        // eg. it could be possible to switch situations, without needing to call the entering situation method.
