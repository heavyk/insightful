...
function Ambition$_resolve (it, cb) {
  ...
  var b
  var next = function () {
    if (arguments.length && typeof (b = arguments[0]) === 'object') daFunk.extend(a[0], b)
    process.nextTick(function () {
      cb.apply(self, a)
    })
  }
...
Ambition.prototype.nowPrepared =
function Ambition$nowPrepared (lastSituation, nextSituation, prepared, args) {
  var self = this
...
function Ambition$now (nextSituation) {
  ...
  if (self.situation && self.situation[0] !== '/' && self.situation !== self.startingSituation) {
    console.warn('WARNING ' + self.namespace + ' is trying to now while already in a now situation: ' + self.situation)
    return self.soon.apply(self, arguments)
  }
  if (self.inTransition) {
    return self.soon.apply(self, arguments)
  }
  console.debug('fsm: now %s -> %s', self.situation, nextSituation)
